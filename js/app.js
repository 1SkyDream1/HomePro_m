(() => {
    ...
    (() => {
        ...
        const titles = document.querySelectorAll(".header-block__title");
        const texts = document.querySelectorAll(".header-block__text");
        if (titles.length) titles.forEach((title => {
            title.dataset.watch = "";
        }));
        if (texts.length) texts.forEach((text => {
            text.dataset.watch = "";
            text.dataset.watchOnce = "";
        }));
        class ScrollWatcher {
            constructor(props) {
                let defaultConfig = {
        ...
        function initBlog(data, start, end) {
            ...
            for (let index = start; index < end; index++) if (index < items.length) {
                const item = items[index];
                buildBlogItem(item);
            } else break;
            if (items.length == document.querySelectorAll(".blog__item").length) {
        ...
        function buildBlogItem(item) {
            ...
            let result = ``;
            result += `<article class="blog__item item-blog">`;
            item.image ? result += `<a href="${item.url}" class="item-blog__image-ibg"><img src="${item.image}" alt="Image"></a>` : null;
            result += `<div class="item-blog__body">`;
            result += `<div class="item-blog__date">${item.date}</div>`;
            result += `<h4 class="item-blog__title">\n\t\t\t\t\t\t<a href="${item.url}" class="item-blog__title-link">${item.title}</a>\n\t\t\t\t\t</h4>`;
            result += `<div class="item-blog__text"><p>${item.text}</p></div>`;
            result += '<div class="item-blog__tags">';
            for (const tag in item.tags) result += `<a href="${item.tags[tag]}" class="item-blog__tag">${tag}</a>`;
            result += `</div>`;
            result += `</div>`;
            result += `</article>`;
            blogItems.insertAdjacentHTML("beforeend", result);
        }
        document.addEventListener("click", documentActions);
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".blog__more")) {
                e.preventDefault();
                const start = document.querySelectorAll(".blog__item").length;
                const end = start + 3;
